...
impl Qwen3Tokenizer {
    pub fn new(model_dir: &Path) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let data = fs::read_to_string(&tokenizer_path)?;
...
mod tests {
    ...
    fn test_gpt2_space_mapping() {
        let decoder = build_gpt2_byte_decoder();
        // Space (0x20 = 32) is NOT in the printable range '!' (33) through '~' (126),
        // so it gets mapped to a Unicode character starting at 256.
        // The exact Unicode codepoint depends on ordering; verify it exists.
        let space_char = {
            ...
            for b in 0u8..=255u8 {
                if !encoder.contains_key(&b) {
                    encoder.insert(b, char::from_u32(n).unwrap());
                    n += 1;
                }
            }
            *encoder.get(&0x20).unwrap()
        };
        assert_eq!(decoder[&space_char], 0x20);
    }
